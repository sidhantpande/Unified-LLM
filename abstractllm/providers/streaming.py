...
class UnifiedStreamProcessor:
    ...
    def _apply_tag_rewriting(self, content: str) -> str:
        ...
        try:
            ...
            response_for_rewriting = GenerateResponse(
                content=content,
                model=self.model_name,
                finish_reason="stop"
            )
            rewritten_response = apply_tool_call_tag_rewriting(response_for_rewriting, self.tool_call_tags)
            return rewritten_response.content
