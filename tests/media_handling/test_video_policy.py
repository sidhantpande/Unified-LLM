...
def test_native_only_allows_video_when_model_capabilities_claim_support(tmp_path: Path) -> None:
    ...
    assert provider.last_media and provider.last_media[0].media_type.value == "video"
...
def test_frames_caption_replaces_video_with_images(monkeypatch, tmp_path: Path) -> None:
    ...
    def fake_extract_video_frames(video_path: Path, *, max_frames: int = 3, frame_format: str = "jpg", output_dir=None):
